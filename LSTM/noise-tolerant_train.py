...
import pandas as pd
dataset = pd.read_csv("GLM_Change_Dataset.csv", engine= "python", header= None, encoding='utf-8', encoding_errors="ignore")
print(dataset.info())
...
#文本批处理
train_iter, val_iter = data.BucketIterator.splits((train, val), batch_size=32, device=device,sort_key=lambda x: len(x.text))
...
class BiLSTMModel(nn.Module):
    def __init__(self, hidden_size, embedding_dim, vocab_size):
        ...
        self.embedding = nn.Embedding(vocab_size, embedding_dim)
        self.encoder = nn.LSTM(input_size=embedding_dim, hidden_size=hidden_size, num_layers=1, bidirectional=True)
        # 输出维度会是 hidden_size * 2，因为是双向的
        self.predictor = nn.Linear(hidden_size * 2, 2)

    def forward(self, x):
        out = self.embedding(x)
        out, (hidden, cell) = self.encoder(out)
        # 将双向 LSTM 的输出拼接起来，然后送入线性层
        output = self.predictor(torch.cat((hidden[-2, :, :], hidden[-1, :, :]), dim=1))
        return output

lstm = Model(hidden_size=100, embedding_dim=300, vocab_size=20002)
# lstm = BiLSTMModel(hidden_size=100, embedding_dim=300, vocab_size=20002)
lstm.to(device)
